        ...
        <div className={styles.container}>
            ...
            <div className={styles.patientContainer}>
                {patients.length !== 0 && patients.map(patient => (
                    <div
                        key={patient?.['medical_ref_number']}
                        className={styles.patientAvatar}
                        onClick={() => navigate(`/patient/${patient?.['medical_ref_number']}`)} // Clicking an avatar now navigates to the patient's page
            ...
            <div className={styles.scheduleContainer}>
                {treatments.length !== 0 && treatments.map(treatment => (
                    <div key={treatment?.["id"]} className={styles.treatmentWrapper}>
                        <Avatar style={{ background: "white", color: "#004AAD" }}>{treatment?.["id"]}</Avatar>
                        <span>{`Treatment session at ${new Date(treatment?.['start_time_scheduled']).toLocaleString()} for ${vals.get(treatment?.['id'])}`}</span>
                        <Button
                            shape={"circle"}
                            style={{ background: "#004AAD" }}
                            onClick={() => navigate("/treatment_session")}
